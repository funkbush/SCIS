...
$stock="600401";
$server="220.178.118.60";
$sd="2016-12-31";//$_POST["sd"];
$ed="2017-01-09";//$_POST["ed"];
...
//#######################*********************************************************************
		...
		usleep(100000*$a/2000);  //根据导入数据块大小来暂停不同的时间
		
		mysql_ping();  //加这个似乎数据库可以一直保持连接，最后断开的原因是socket
		//echo mysql_ping()."<br>";
		...
		if($fhz==1)
		{
		   ...
		   echo "<script>setDownloaded(".($k+1).",".count($arrdate).")</script>";//在前台显示已经下载文件大小
        }
        elseif($m==0 && $fzh==0)
        {
		   //ob_flush();
...
//echo "<script>setDownloaded(".$k.",".count($arrdate).")</script>";//在前台显示已经下载文件大小 
echo mysql_stat(); 
?>
